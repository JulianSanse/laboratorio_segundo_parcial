...
static int addNode(LinkedList* this, int nodeIndex,void* pElement)
{
    ...
    if(this != NULL && len >= 0 && nodeIndex >= 0 && nodeIndex <= len){
    	...
    	nodoExistente = getNode(this, nodeIndex);
    	if(nodoAux != NULL){
    		nodoAux->pElement= pElement;
    		nodoAux->pNextNode = nodoExistente;
    		...
    		if(nodeIndex == 0){
    		    ...
    		    nodoAnterior->pNextNode = nodoAux;
    		}
...
int ll_remove(LinkedList* this,int index)
{
    ...
    if(this != NULL && index >= 0 && index < len && nodoAEliminar != NULL){
    	if(index == 0){
...
int ll_push(LinkedList* this, int index, void* pElement)
{
    ...
    if(this != NULL && len >= 0 && index >= 0 && index <= len){
        	nodoAux = (Node*) malloc(sizeof(Node));
        	nodoExistente = getNode(this, index);
        	if(nodoAux != NULL){
        		nodoAux->pElement= pElement;
        		nodoAux->pNextNode = nodoExistente;
...
LinkedList* ll_subList(LinkedList* this,int from,int to)
{
    ...
    if(this != NULL && from >= 0 &&  to > from && to <= len){
    	...
    	if(cloneArray != NULL){
    		Node* nodoACopiar;
    		for(i = from; i < to; i ++){
    			nodoACopiar= getNode(this, i);
    		    if(nodoACopiar!= NULL){
    		    	if(i == from){
    		    		cloneArray->pFirstNode = nodoACopiar;
    		    	}else{
    		    		nodoACopiar->pNextNode = nodoACopiar;
    		    	}
    		    	cloneArray->size++;
    		    	}
    ...
    if(this!=NULL && pFunc!= NULL && len >= 0 && (order == 0 || order == 1)){
    	for(i=0; i<len; i++){
    		for (j= i+1; j < len; j++){
    			...
    			if(order == 1){
    				if((pFunc(elementoUno, elementoDos)) > 0){
